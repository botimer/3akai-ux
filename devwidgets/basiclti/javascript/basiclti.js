	...
	var renderTemplate = function(container){
        if (showSettings) {
            ...
            $('button[name=save_settings]', rootel).click(function(e) {
                var settings = {
                    ltiurl: $('input[name=ltiurl]', rootel).val(),
                    ltikey: $('input[name=ltikey]', rootel).val(),
                    ltisecret: $('input[name=ltisecret]', rootel).val(),
                    frame_height: $('input[name=frame_height]', rootel).val(),
                    debug: $('input[name=debug]:checked', rootel).length == 1,
                    release_names: $('input[name=release_names]:checked', rootel).length == 1,
                    custom_params: $('input[name=custom_params]', rootel)
                };
            ...
            var fr = $('<iframe src="' + launchDataUrl + '" id="basiclti_launch_frame" height="800" width="640" border="0"></iframe>');
